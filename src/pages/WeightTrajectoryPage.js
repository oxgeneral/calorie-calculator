...
const WeightTrajectoryPage = () => {
  ...
  const fastDeficit = isWeightLoss ? 1000 : 1000; // дополнительный дефицит/профицит 500 ккал (всего 1000)
  ...
  const weeklyChange = state.weightLossGoal ? 0.5 : 0.25;
    ...
    ? (
      <div className="mt-3 text-sm text-gray-700">
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li>Мы рекомендуем научный подход для более устойчивых долгосрочных результатов и лучшего сохранения мышечной массы.</li>
        </ul>
      </div>
    )
    : (
      <div className="mt-3 text-sm text-gray-700">
        <h4 className="font-semibold mb-1">Что показывает этот график:</h4>
        <ul className="list-disc pl-5 space-y-1">
          <li><span className="text-blue-600 font-medium">Синяя линия</span> - научный подход с постепенным набором 0.25 кг в неделю, который максимизирует прирост мышечной массы и минимизирует накопление жира.</li>
          <li><span className="text-red-600 font-medium">Красная пунктирная линия</span> - более агрессивный подход с профицитом 1000 ккал, который дает быстрый набор веса (0.5 кг в неделю), но приводит к большему накоплению жировой ткани.</li>
          <li>Научный подход рекомендуется большинству людей, особенно тем, кто не является начинающим в тренировках.</li>
        </ul>
      </div>
    );
